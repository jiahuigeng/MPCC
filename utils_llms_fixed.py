...
class InternOmniLLM(LocalHuggingFaceLLM):
    ...
    def load_model(self):
        """修复后的 load_model 方法 - 解决 KeyError: 'architectures' 问题"""
        if self.model is None:
            print(f"Loading {self.model_name} from {self.hf_path}...")
            try:
                import sys
                ...
                # === 修复 2: 降低日志级别避免触发 __repr__ 中的 KeyError ===
                original_verbosity = transformers.logging.get_verbosity()
                transformers.logging.set_verbosity_error()
                
                try:
                    # === 修复 3: 处理 KeyError: 'architectures' ===
                    ...
                    try:
                        ...
                        print("→ Applying configuration patch...")
                        
                        # 查找并修补配置模块
                        patched = False
                        for module_name in list(sys.modules.keys()):
                            if 'InternOmni' in module_name and 'configuration' in module_name:
                                module = sys.modules[module_name]
                                
                                # 修补 InternVLChatConfig
                                if hasattr(module, 'InternVLChatConfig'):
                                    original_init = module.InternVLChatConfig.__init__
                                    
                                    def patched_init(self, vision_config=None, llm_config=None, **kwargs):
                                        # 确保 llm_config 包含 'architectures' 键
                                        if llm_config is not None and isinstance(llm_config, dict):
                                            if 'architectures' not in llm_config:
                                                llm_config['architectures'] = ['InternLM2ForCausalLM']
                                        original_init(self, vision_config, llm_config, **kwargs)
                                    
                                    module.InternVLChatConfig.__init__ = patched_init
                                    print(f"✓ Patched {module_name}.InternVLChatConfig")
                                    patched = True
                        
                        if not patched:
                            print("⚠ Warning: Could not find config module to patch")
                        ...
                        config_loaded = True
                        print("✓ Configuration loaded after patching")

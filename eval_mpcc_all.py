...
TASK_CONFIG = {
    "calendar": {
        ...
        "prompt": "You are solving MPCC Calendar Planning. Read the calendar image carefully and answer EXACTLY in the required output format.\n\n"
    },
    "meeting": {
    ...
    "flight": {
        "keys": ["flight_way"],
        "prompt": "You are solving MPCC Flight Planning. Read the flight schedule and price information carefully and answer EXACTLY in the required output format.\n\n"
    }
}

def determine_task(path: str) -> str:
    lower_path = path.lower()
    if "calendar" in lower_path: return "calendar"
    if "meeting" in lower_path: return "meeting"
    if "flight" in lower_path: return "flight"
    return "calendar"
...
def extract_answer_json(text: str, keys: List[str]) -> Optional[Dict[str, Any]]:
    ...
    # This is harder for generic keys, but we can try basic "key": "value"
    extracted = {}
    try:
        for k in keys:
...
def exact_match(pred: str, gold: Any, keys: List[str]) -> Tuple[bool, bool]:
...
def load_parquet(path: str) -> pd.DataFrame:
    ...
    # Check essential columns only if needed, but MPCC Parquet structure is consistent
    return df
...
def evaluate_mpcc(
    ...
    task_type = determine_task(parquet_path)
    ...
    if hasattr(model, "prompt_prefix") and not model.prompt_prefix:
        model.prompt_prefix = config["prompt"]
    elif hasattr(model, "prompt_prefix"):
        # If model has a default prompt prefix, maybe we should override it?
        # The OpenAIVisionModel instance was created with empty prompt_prefix in main if we change main
        model.prompt_prefix = config["prompt"]
    ...
    for i, row in df.iterrows():
        ...
        # Determine image path metadata for logging
        image_path_meta = ex.get("image_path")
        ...
        rows_out.append({
            "index": ex.get("index"),
            "image_path": str(image_path_meta),
            "question": q,
            "answer": gold,
            "predicted": pred,
    ...
    print(f"=== MPCC Evaluation ({task_type}) ===")
    ...
    if save_predictions:
        ...
        # Construct CSV filename: original filename + "_baseline.csv" (or provided path)
        # User asked: "add to the end of the code a save to corresponding csv, ... save image_path, question, answer, predicted, Extracted_Pred"
        # And "use default prompt save name add baseline"
        
        # I'll rely on the caller to provide a path, OR I modify the path here.
        # But for the .sh script, I can pass a path like "results/calendar_easy_baseline.csv"
        
        # If the user passed a path that ends in .jsonl or .csv, I can use it.
        # Let's just save to CSV using pandas.
        ...
        # Keep only requested columns + identification
...
def main():
    ...
    parser.add_argument("--save", type=str, default=None, help="Path to save output (CSV will be derived from this)")
    ...
    if args.model == "dummy":
        ...
        # Note: prompt_prefix is set inside evaluate_mpcc based on task
